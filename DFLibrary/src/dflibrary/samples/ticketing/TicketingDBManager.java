 ...
 *
 */
public class TicketingDBManager {
	
		/**
		 * 
		 */
		public TicketingDBManager(TicketingView view){
		...
		private void setConnection(){
	        ...
	        try {
	            con = DriverManager.getConnection("jdbc:mysql://" + getIP() + ":" + PORT + "/" + "chivas_db", "chivasweb", "mysqlchivasweb");
	        } catch (Exception e){
		...
		public int[] getAvTickets(int event_id){
	        ...
	        try {
	            ...
	            Statement s = con.createStatement();
	            ResultSet rs = s.executeQuery ("SELECT ticket_id FROM tickets WHERE event_id = " + event_id + " AND user_id = 0");
		...
		public int[] getAvTickets(String description, String date){
	        ...
	        try {
	            ...
	            rs = s.executeQuery("SELECT ticket_id FROM tickets WHERE event_id = " + event_id + " AND user_id = 0");
		...
		public Ticket getTicket(int ticket_id){
	        ...
	        try {
	            ...
	            Statement s = con.createStatement();
	            ResultSet rs = s.executeQuery ("SELECT * FROM tickets WHERE ticket_id = " + ticket_id);
		...
		public int getUserId(String uid){
			 ...
			 try {
		           ...
		           Statement s = con.createStatement();
		           ResultSet rs = s.executeQuery ("SELECT user_id FROM users WHERE card_id = '" + uid + "'");	            
		...
		public void registerTicket(int ticket_id, int user_id){
	        ...
	        try {
	            ...
	            Statement s = con.createStatement();
	            s.executeUpdate ("UPDATE tickets SET user_id = " + user_id + " WHERE ticket_id = " + ticket_id);
		...
		public static final int TICKETS_STATE = 10;
