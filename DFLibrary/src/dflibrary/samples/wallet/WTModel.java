...
public class WTModel {
	 ...
	 * @return
	 */
	protected Bank[] getBanks(){ return bdb.getBanks(); }
	
	/**
	 * 
	 * @return
	 */
	protected Event[] getEvents(){ return tdb.getEvents(); }
	 ...
	 * @return
	 */
	protected String[] getDates(String event){ return tdb.getDates(event); }
	...
	private void doAppTask(){
		...
		if(getCurrApp() == WTApp.FACTORY_RESET){
			...
			if(getView().getIBAllowed()){	
				...
				getView().log("Wallet configuration not allowed until a valid initial balance is provided\n");
			}
			...
			if(getView().getRFAllowed()){
				...
				getView().log("Wallet refilling not allowed until a valid amount is provided\n");
			}
		}		
			...
			if((tdAllowed)||(tcAllowed)){
				getView().log("Starting the ticketing app configuration process...\n");
				tickConfig(tdAllowed, tcAllowed);
			}
			else{
				getView().log("Ticketing app configuration process not allowed until at least one option is selected\n");
			}
	...
	private void factoryReset(){
		...
		try{
			...
			getView().log("Starting the factory reset process in the inserted card...\n");
	...
	private void format(){
		...
		getView().log("Formating card to delete all existing applications and files in the card...");
		
		DFResponse res = df.format(bank.getBankID());
		getView().log(res);
		getView().log("");
		
		if(!res.isOk()) throw new RuntimeException("Card format not successfully finished");
	...
	private void setDefaultCardMasterKey(){
		...
		if(!res.isOk()) throw new RuntimeException("Card Master Key wasn't successfully set to its default value");
	}
	...
	private void setMasterKey(){
		...
		if(!res.isOk()) throw new RuntimeException("Card Master Key wasn't successfully set");
	...
	protected void createWalletApp(){
		...
		if(!res.isOk()) throw new RuntimeException("Wallet Application wasn't successfully created");
	...
	protected void configWalletApp(){
		...
		getView().log("Configuring the recently created Wallet application...");
		
		DFResponse res;
		getView().log("Configuring the Wallet application keys...\n");		
		res = df.configWalletAppKeys(WALLET_AID, bank.getBankID());
		if(!res.isOk()) throw new RuntimeException("Wallet Application wasn't properly configured:  Key configuration error");
		...
		if(!res.isOk()) throw new RuntimeException("Wallet Application wasn't properly configured: File creation error");
		...
		try{
			...
			if(tdAllowed){
				...
				if((!isApp) && (!tcAllowed)){
					...
					getView().log("Ticketing application doesn't exists in the card yet");
					throw new RuntimeException("Ticketing app deletion didn't succeed. \nThe application doesn't exist in the card yet");
				}
			...
			if(tcAllowed){
				...
				if((isApp) && (!tdAllowed)){
					...
					getView().log("Ticketing application already exists in the card");
					throw new RuntimeException("Ticketing app creation didn't succeed. \nThe application already exists in the card");
				}
	...
	protected void configTickApp(){
		...
		if(!res.isOk()) throw new RuntimeException("Ticketing application wasn't properly configured:  Key configuration error");
		...
		if(!res.isOk()) throw new RuntimeException("Ticketing application wasn't properly configured: File creation error");
	...
	private void purchase(){
		...
		try{
			...
			//Checking if the Wallet Application is present in the card in order to pay the purchase
			
			selectMasterApp();
			checkWalletApp();
			
			//Checking if the Ticketing Application and the Tickets file are present in the card
			
			selectMasterApp();
			boolean isApp = checkTickApp();
			if(!isApp) throw new RuntimeException("Ticketing app not present in the card");			
			
			selectTickApp();
			boolean isFile = checkTickFile();
			if(!isFile) throw new RuntimeException("Tickets file not present in the card");
			
			//Checking if there is enough space in the Tickets file for another ticket
			
			boolean freeRecords = checkTicketFileSpace();
			if(!freeRecords) throw new RuntimeException("There is not enough space for another ticket");
...
//			
	...
	private void selectMasterApp(){
		...
		if(!res.isOk()) throw new RuntimeException("Selection of Card Master Application didn't succeed");
	}
	...
	private void selectWalletApp(){
		...
		if(!res.isOk()) throw new RuntimeException("Selection of Wallet Application didn't succeed");
	...
	private void selectTickApp(){
		...
		if(!res.isOk()) throw new RuntimeException("Selection of Ticketing Application didn't succeed");
	...
	private void searchTicket(int user_id){
		...
		if(!res.isOk()) throw new RuntimeException("Could not retreive Ticket file settings");
		...
		if(currNumOfTickets == 0) throw new RuntimeException("No available tickets");
...
//		
		if(!event_found) throw new RuntimeException("No ticket available for this event");
		if(ticket_used) throw new RuntimeException("All tickets available for this event are already used");
...
//		markUsed(ticket);
	...
	private String bankConnection(int bankID, String order, boolean legacy){
		...
		try{	
			...
			try{
				...
				finally{ s.close(); getView().log("Connection closed by remote server");}
			}
	 ...
	 * @return
	 */
	protected WTView getView(){ return this.view; }
	
	/**
	 * 
	 * @return
	 */
	protected WTApp getCurrApp(){ return this.currApp; }
	 ...
	 * @return
	 */
	protected Bank getBank(){ return this.bank; }
		
	/**
	 * 
	 * @param bank
	 */
	protected void setBank(Bank bank){ this.bank = bank; }
	
	/**
	 * 
	 * @return
	 */
	protected int getCredit(){ return this.credit; }
	
	/**
	 * 
	 * @param i
	 */
	protected void setCredit(int i){ this.credit = i; }
